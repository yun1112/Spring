...
public class PhoneBookManager {
	final PhoneInfor[] pBooks;// 배열 개수 고정
...
//		cnt++;
	...
	// 배열에서 이름을 기준으로 검색 후 index값을 반환하는 기능
	int searchIndex(String name) {
		int searchIndex = -1;// 검색결과 없을 경우 나타내기 위해 -1로 초기화함
	...
	// 검색 : 이름을 기준으로 데이터를 찾아서 해당 데이터의 정보를 출력
	// 배열의 요소(PhoneInfor)의 name속성 값으로 배열의 요소(해당하는 참조값 찾아야 함)
	void searchInfor() {
